...
ProgramStart:
	...
	mov r2,#0x403 ;The 4 enables bg layer 2, the 3 enables screenmode 3 which is bitmap
	str r2,[r4] ;This is soring the value in r2 to the address described in r4
...
PrintChar:
	STMFD sp!,{r0-r12,lr} ;This stores the current registers and the link register which contains the return address. The ! after ap indicates that the stack pointer will be changes after the operation
	mov r4,#0
	mov r5,#0
	mov r3,#CursorX
	ldrB r4,[r3] ;this loads the cursor x position into register 4
	mov r3,#CursorY
	ldrB r5,[r3] ;Load register byte
	
	mov r3, #0x06000000 ;This is the VRAM base address
	
	mov r6,#8*2 ;There are two bytes per pixel and 8 pixels per character horizontally.
	mul r2,r4,r6 ;Take the cursor x and multiply it by 16
	add r3,r3,r2 ;add the offset that was calculated in the previous line to the base VRAM address
	
	mov r4,#240*8*2 ;Next we begin to calculate teh Y Offset. There are 240 pixels per line and 2 bytes for each pixel. The font is 8 pixels tall
	mul r2,r5,r4
	add r3,r3,r2 ;this will give us the true offset of the character we wish to draw inside of register r3
	
	;Next we find the address of the character we want within our font, we are only including characters 32-128 for this example
	adr r4,BitmapFont ;this uses address relative to get the address of the bitmap font defined in this program.
	
	sub r0,r0,#32 ;we are removing the offset of the first 32 characters
	add r4,r4,r0,asl #3 ;r4 = r4 + (r0 << 3) since each character is 8 bytes. equivalent to multiplying by 2^3=8
	
	mov r1,#8 ; Use 8 lines
DrawLine:
	mov r7,#8 				;8 pixels per line
	ldrb r8,[r4],#1			;Load Letter
	mov r9,#0b100000000		;Mask
	
	mov r2, #0b1111111010100000 ;this is a blue color in the 1555 color format, argb
DrawPixel:
	tst r8, r9 ;this tests the two bits
	strneh r2, [r3] ;This is a conditional store register, not equal, half-word(16 bit)
	;It stores the first 16 bits of register 2 into the address of r3 if r8 != r9
	add r3,r3,#2;Move to the next pixel
	mov r9,r9,ror #1 ;This rotates the value in r9 to the right by one. Each bit in the bitmap is cycled through in this manner.
	subs r7,r7,#1 ;subtract one and set flags
	bne DrawPixel ; branch if not equal. This will not branch if r7=0 which is being used to iterate.
	
	add r3,r3,#480-16 ;move down a line 240*2 - 1 character(16)
	subs r1,r1,#1
	bne DrawLine
LineDone:
	mov r3, #CursorX
